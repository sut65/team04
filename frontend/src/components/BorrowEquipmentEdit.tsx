...
interface BorrowEquipment {
  Cancle: () => void;
  Data: BorrowEquipmentInterface | undefined;
}
...
function EditBorrowEquipment({ Cancle, Data }: BorrowEquipment) {
  ...
  const [borrowequipment, setBorrowEquipment] = useState<Partial<BorrowEquipmentInterface>>({
    ...
    Amount_BorrowEquipment: Data?.Amount_BorrowEquipment,
    BorrowEquipment_Day:    Data?.BorrowEquipment_Day,
        ...
        >
          <Box sx={{ paddingX: 2, paddingY: 1 }}>
            <Typography
              ...
              gutterBottom
            >
              แก้ไขข้อมูลบันทึกการยืมอุปกรณ์ ลำดับที่ {borrowequipment.ID}
            </Typography>
          </Box>
        </Box>
          ...
          <Grid item xs={6}>
            <FormControl fullWidth variant="outlined">
              ...
              <Select
                ...
                inputProps={{
                  name: "UserID", //เอาไว้เข้าถึงข้อมูล
                }}
              >
                ...
                {user.map((item: UserInterface) => (
                  <option value={item.ID} key={item.ID}>
                    {item.Name}
                  </option> //key ไว้อ้างอิงว่ที่1ชื่อนี้ๆๆ value: เก็บค่า
                ))}
              </Select>
            </FormControl>
          </Grid>
...
export default EditBorrowEquipment;
