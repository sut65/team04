...
interface Forfeit {
  Cancle: () => void;
  Data: ForfeitInterface | undefined;
}

function ForfeitEdit({ Cancle, Data }: Forfeit) {
  const [Pay_Date, setPay_Date] = useState<Date | null>();
  const [forfeit, setForfeit] = useState<
    Partial<ForfeitInterface>
  >({
    ID: Data?.ID,
    ReturnBookID: Data?.ReturnBookID,
    Pay: Data?.Pay,
    PaymentID: Data?.PaymentID,
    Pay_Date: Data?.Pay_Date,
    Note: Data?.Note,
    ModulateNote: Data?.ModulateNote,
  ...
  const GetAllReturnBook = async () => {
    ...
    fetch(apiUrl, requestOptions)
      ...
      .then((res) => {
        console.log("ReturnBook", res.data);
        if (res.data) {
