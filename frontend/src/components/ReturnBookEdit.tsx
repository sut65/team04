...
interface ReturnBook {
  Cancle: () => void;
  Data: ReturnBookInterface | undefined;
}
...
function EditReturnBook({ Cancle, Data }: ReturnBook) {
  ...
  const [returnbook, setReturnBook] = useState<Partial<ReturnBookInterface>>({
          ...
          Late_Number:     Data?.Late_Number,
          Book_Condition:  Data?.Book_Condition,
          LostBookID:      Data?.LostBookID,
          LibrarianID:     Data?.LibrarianID,
          BorrowBookID:    Data?.BorrowBookID,
  ...
  function submit() {
    let data = {
      ...
      ID:               Number(returnbook.ID),
      Current_Day:      current_day,  //?.toISOString()
  ...
  return (
    <Container maxWidth="md">
      ...
      <Paper>
        ...
        <Box sx={{ bgcolor: '#d0f8ce' }} flexGrow={1}>
